...
const Contact = () => {
  ...
  return (
    <div className="flex sm:gap-6 gap-8 align-bottom">
      {
        contactLinks.map((link, index) => (
          <a key={index} href={link.href} target="_blank" alt={link.alt}>
            <img src={link.imgSrc} className='h-8 w-8 sm:h-4 sm:w-4 object-contain transition-transform duration-200 ease-in-out hover:scale-125 bg-[#2A2929] p-1 sm:p-0 rounded-full' />
          </a>
        ))
      }
    </div>
